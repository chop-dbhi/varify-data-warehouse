...
def cohort_bulk_add(orm, cohort, samples):
    _objs = []
    loaded = 0
    for obj in iter(samples):
        _objs.append(orm['samples.CohortSample'](object_set=cohort, set_object=obj))
        loaded += 1
    orm['samples.CohortSample'].objects.bulk_create(_objs)
    cohort.count += loaded
    cohort.save()
...
class Migration(DataMigration):
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 11, 27, 17, 25, 30, 684643)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 11, 27, 17, 25, 30, 684424)'}),
